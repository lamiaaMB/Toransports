  ...
  if (connection_speaks_cells(conn) &&
    ...
    CONN_LOG_PROTECT(conn,
                     result = buf_read_from_socket(conn->inbuf, conn->s,
                                                   ...
                                                   &reached_eof,
                                                   socket_error));
	log_notice(LD_GENERAL,"Connection type :   %d",conn->type);

    if(conn->type == CONN_TYPE_OR){
    	tor_addr_t * t;
    	TO_OR_CONN(conn)->chan->get_remote_addr(TO_OR_CONN(conn)->chan,t);
    	log_notice(LD_GENERAL,"Connection channel address :   %s",t->addr);
